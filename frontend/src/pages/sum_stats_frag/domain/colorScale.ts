...
import {
  ...
  toShortCol,
  toShortCols,
} from "@/pages/sum_stats_frag/domain/columns";
import { getNumericValue, keyFromCols } from "@/pages/sum_stats_frag/domain/data";
import type {
  ColorScaleResult,
  DataRow,
  LegendItem,
} from "@/pages/sum_stats_frag/domain/types";
...
export const buildColorScale = <T extends DataRow>({
  ...
  const hasExplicitNoColor = colorColumns.length === 1 && colorColumns[0].trim().length === 0;
  ...
  const customMap =
    colorColumnsShort.length === 1 ? customDiscreteColorMaps[firstColorColumn] : undefined;
  ...
  const ordinalScale = d3.scaleOrdinal<string, string>(d3.schemeCategory10).domain(globalColorOrder);
